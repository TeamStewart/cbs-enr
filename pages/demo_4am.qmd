...
data22_final = read_csv("~/Dropbox (MIT)/Research/2024 Election Results/22_general/GA_allReports.csv") |> 
  filter(str_detect(race_name, "US Senate")) |> 
  filter(timestamp == max(timestamp), .by = c(state, jurisdiction, precinct_id)) |> 
  filter(timestamp > ymd("2022-10-15")) |> 
  select(state, county = jurisdiction, precinct_22 = precinct_id, votes_22 = precinct_total) |> 
  summarize(
    votes_precFinal_22 = sum(votes_22),
    .by = c(state, county, precinct_22)
  ) |> 
  mutate(
    county = str_to_upper(county),
...
data20 = read_csv("~/Dropbox (MIT)/Research/MEDSL Precinct Data/2020-ga-precinct-general.csv") |> 
...
precinct_merge = intersection |> 
  ...
  mutate(
    across(c(votes_20, votePct_dem_20, votePct_rep_20, votePct_diff_20, votes_precFinal_20), ~ mean(.x, na.rm=TRUE)),
...
estimates = precinct_merge |> 
  ...
  mutate(
    votes_22_demEst = case_when(
      ...
      votes_22 > 0 ~ votes_22,
      .default = votes_20 * (dem_med / votePct_dem_20) * turn_med
    ),
    votes_22_demBot = case_when(
      reported_all ~ votes_22,
      # keep early or mail votes that are already in
      votes_22 > 0 ~ votes_22,
      .default = votes_20 * (dem_bot / votePct_dem_20) * turn_bot
    ),
    votes_22_demTop = case_when(
      reported_all ~ votes_22,
      # keep early or mail votes that are already in
      votes_22 > 0 ~ votes_22,
      .default = votes_20 * (dem_top / votePct_dem_20) * turn_top
    ),
