...
# read_csv("~/Downloads/ga22_cleaned.csv") |> filter(office == "US SENATE", stage == "GEN", mode != "TOTAL") |> summarize(votes = sum(votes), .by = mode)
...
summaries_byCounty_byMode = estimates |> 
  summarize(
    ...
    demShare_lower = demVotes_lower / votes_total_22_estimate,
    demShare_estimate = demVotes_estimate / votes_total_22_estimate,
    demShare_upper = demVotes_upper / votes_total_22_estimate,
    repShare_lower = repVotes_lower / votes_total_22_estimate,
    repShare_estimate = repVotes_estimate / votes_total_22_estimate,
    repShare_upper = repVotes_upper / votes_total_22_estimate,
...
- We estimate the **final Election Day turnout** will be `r pull(filter(summaries_byMode, vote_mode == "Election Day"), votes_total_22_estimate)` with uncertainty bounds [`r pull(filter(summaries_byMode, vote_mode == "Election Day"), votes_total_22_lower)` -- `r pull(filter(summaries_byMode, vote_mode == "Election Day"), votes_total_22_upper)`]
...
summaries_byCounty_byMode |> 
  mutate(
    ...
    across(c(contains("swing"), contains("Share")), ~ scales::label_percent()(.x)),
    across(votes_total_20:repVotes_upper, ~ round(.x, digits = 0) |> format(big.mark = ",") |> as.character())
  ) |> 
  mutate(
    votes_total_22 = case_when(
      ...
      .default = glue("{votes_total_22_estimate} [{votes_total_22_lower} to {votes_total_22_upper}]")
    ),
    votes_dem_22 = case_when(
      demVotes_estimate == demVotes_lower & demVotes_estimate == demVotes_upper ~  demVotes_estimate,
      .default = glue("{demVotes_estimate} [{demVotes_lower} to {demVotes_upper}]")
    ),
    votes_rep_22 = case_when(
      repVotes_estimate == repVotes_lower & repVotes_estimate == repVotes_upper ~  repVotes_estimate,
      .default = glue("{repVotes_estimate} [{repVotes_lower} to {repVotes_upper}]")
    ),
    swing_22 = case_when(
      is.na(swing_estimate) | vote_mode == "Provisional" ~ NA,
      swing_estimate == swing_lower & swing_estimate == swing_upper ~ swing_estimate,
      .default = glue("{swing_estimate} [{swing_lower} to {swing_upper}]")
    )
  ) |> 
  select(county, vote_mode, timestamp, votes_total_20, votes_total_22:swing_22) |> 
  gt(
    # groupname_col = "county", rowname_col = "vote_mode"
  ) |> 
  # tab_stubhead("Vote Mode") |> 
  ...
  cols_label(
    ...
    votes_total_22 = "2022 Votes",
    votes_dem_22 = "2022 Dem Votes",
    votes_rep_22 = "2022 Rep Votes",
    swing_22 = "2022 Swing"
  ) |> 
  cols_width(
    timestamp ~ px(165)
