...
data24 = read_csv(reports$path[10]) |> 
  ...
  select(state, county = jurisdiction, precinct_24 = precinct_id, candidate_name, vote_mode, precinct_total) |> 
  mutate(
  ...
  mutate(
    precinct_pct = ifelse(precinct_votes > 0, sum(precinct_total) / precinct_votes, NA),
    .by = c(state, county, precinct_24)
  ) |> 
  # mutate(
...
data22 = read_csv("~/Dropbox (MIT)/Research/cbs-enr/data/input/GA/results_2022.csv") |> 
  ...
  inner_join(top_contests_22) |>
  filter(mode != "TOTAL") |> 
  mutate(
    votePct_byMode = votes / sum(votes, na.rm = TRUE),
    .by = c(state, county, precinct_22)
  ) |> 
  select(state, county, precinct_22, mode, votePct_byMode)
...
shp22 = read_sf("~/Dropbox (MIT)/Research/cbs-enr/data/shapefiles/ga-precincts2022-shape/") |> 
  ...
  drop_na(county) |> 
  mutate(precinct_22 = str_to_upper(precinct_22), state = "GA") |> 
...
precinct_merge = intersection |> 
  ...
  select(state, county, precinct_22, biden_20_byPrec, votes_20_byPrec) |> 
...
merged = data24 |> 
  filter(str_detect(candidate_name, "Biden")) |> 
  left_join(voteHist_byPrecinct, join_by(state, county, precinct_24), relationship = "many-to-many") |> 
