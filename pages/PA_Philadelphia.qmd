---
title: "Pennsylvania -- Philadelphia"
date: now
...
tar_read(model_PA_Philadelphia) |> list2env(envir = .GlobalEnv) |> invisible()
...
`r sum(pull(data_history, votes_precTotal_24))` **total** votes are in, `r 100*sum(pull(data_history, votes_precTotal_24)) / 743966`% of the 2020 total
...
`r sum(pull(filter(data_history, vote_mode=="Election Day"), votes_precTotal_24))` **Election Day** votes are in, `r 100*sum(pull(filter(data_history, vote_mode=="Election Day"), votes_precTotal_24)) / 357668`% of the 2020 total
...
tar_read(plot_voteShare_PA_Philadelphia)
...
tar_read(plot_margin2020_PA_Philadelphia)
...
tar_read(plot_votesEDay_PA_Philadelphia)
...
tar_read(plot_votesAll_PA_Philadelphia)
...
tar_read(tbl_countyMode_PA_Philadelphia)
...
tar_read(tbl_county_PA_Philadelphia)
