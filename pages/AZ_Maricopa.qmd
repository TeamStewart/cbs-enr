...
tar_read(model_AZ_Maricopa) |> list2env(envir = .GlobalEnv) |> invisible()
...
`r sum(pull(data_history, votes_precTotal_24))` **total** votes are in, `r 100*sum(pull(data_history, votes_precTotal_24)) / 2068144`% of the 2020 total
...
`r sum(pull(filter(data_history, vote_mode=="Election Day"), votes_precTotal_24))` **Election Day** votes are in, `r 100*sum(pull(filter(data_history, vote_mode=="Election Day"), votes_precTotal_24)) / 166315`% of the 2020 total
...
tar_read(plot_voteShare_AZ_Maricopa)
...
tar_read(plot_margin2020_AZ_Maricopa)
...
tar_read(plot_votesEDay_AZ_Maricopa)
...
tar_read(plot_votesAll_AZ_Maricopa)
...
tar_read(tbl_countyMode_AZ_Maricopa)
...
tar_read(tbl_county_AZ_Maricopa)
