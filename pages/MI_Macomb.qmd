...
tar_read(model_MI_Macomb) |> list2env(envir = .GlobalEnv) |> invisible()
...
`r sum(pull(data_history, votes_precTotal_24))` **total** votes are in, `r 100*sum(pull(data_history, votes_precTotal_24)) / 494219`% of the 2020 total
...
`r sum(pull(filter(data_history, vote_mode=="Election Day"), votes_precTotal_24))` **Election Day** votes are in, `r 100*sum(pull(filter(data_history, vote_mode=="Election Day"), votes_precTotal_24)) / 207208`% of the 2020 total
...
tar_read(plot_voteShare_MI_Macomb)
...
tar_read(plot_margin2020_MI_Macomb)
...
tar_read(plot_votesEDay_MI_Macomb)
...
tar_read(plot_votesAll_MI_Macomb)
...
tar_read(tbl_countyMode_MI_Macomb)
