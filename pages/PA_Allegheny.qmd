...
tar_read(model_PA_Allegheny) |> list2env(envir = .GlobalEnv) |> invisible()
...
`r sum(pull(data_history, votes_precTotal_24))` **total** votes are in, `r 100*sum(pull(data_history, votes_precTotal_24)) / 1667651`% of the 2020 total
...
`r sum(pull(filter(data_history, vote_mode=="Election Day"), votes_precTotal_24))` **Election Day** votes are in, `r 100*sum(pull(filter(data_history, vote_mode=="Election Day"), votes_precTotal_24)) / 364032`% of the 2020 total
...
tar_read(plot_voteShare_PA_Allegheny)
...
tar_read(plot_margin2020_PA_Allegheny)
...
tar_read(plot_votesEDay_PA_Allegheny)
...
tar_read(plot_votesAll_PA_Allegheny)
...
tar_read(tbl_countyMode_PA_Allegheny)
...
tar_read(tbl_county_PA_Allegheny)
