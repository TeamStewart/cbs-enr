...
tar_read(model_PA_Delaware) |> list2env(envir = .GlobalEnv) |> invisible()
...
`r sum(pull(data_history, votes_precTotal_24))` **total** votes are in, `r 100*sum(pull(data_history, votes_precTotal_24)) / 330150`% of the 2020 total
...
`r sum(pull(filter(data_history, vote_mode=="Election Day"), votes_precTotal_24))` **Election Day** votes are in, `r 100*sum(pull(filter(data_history, vote_mode=="Election Day"), votes_precTotal_24)) / 193268`% of the 2020 total
...
tar_read(plot_voteShare_PA_Delaware)
...
tar_read(plot_margin2020_PA_Delaware)
...
tar_read(plot_votesEDay_PA_Delaware)
...
tar_read(plot_votesAll_PA_Delaware)
...
tar_read(tbl_countyMode_PA_Delaware)
...
tar_read(tbl_county_PA_Delaware)
