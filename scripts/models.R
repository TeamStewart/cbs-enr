...
execute_model <- function(data, state, jurisdiction, type, timestamp, target_office){
  # Check: if we have no votes reported, then don't model
  if(sum(data$precinct_total) == 0){
    return(NULL)
  } else{
    if(state == 'AZ' & jurisdiction == 'MARICOPA' & target_office == "US SENATE-Republican"){
      ...
      #### For testing
      #unique_precincts <- unique(data$precinct_id)
      #random_ids <- sample(unique_precincts,size = round(length(unique_precincts)*0.2))
      #data <- data |> mutate(precinct_total = ifelse(precinct_id %in% random_ids & vote_mode %in% c('Election Day','Provisional'), sample(0:400, n(), replace = TRUE), precinct_total))
      ...
      message('Not a model we are running.')
    }
  }
}
...
run_model <- function(df, model_type, dv, covars, weight, target_mode){
  ...
  # Check: if we have no values for DV, then return 0 for all observations
  if(sum(mode[,dv],na.rm = T) == 0){
    mode$model_estimate <- 0
  } else {
    # Run model
