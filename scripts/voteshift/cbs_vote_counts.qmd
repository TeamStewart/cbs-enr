...
hist_22 <- read_csv("~/Downloads/2022 County History.csv") |>
  janitor::clean_names() |>
...
write_csv(hist_22_merge, "~/Downloads/voteshift_2022_byCounty_raw.csv")
write_csv(hist_22_table, "~/Downloads/voteshift_2022_byCounty_rawPercent.csv")

hist_22_table |> 
  mutate(across(`0-10%`:`98-99.9%`, ~ `100%` - .x)) |>
  write_csv("~/Downloads/voteshift_2022_byCounty_relativePercent.csv")
...
hist_20 <- bind_rows(
  read_csv("~/Downloads/2020 Pres County History/2020 Pres County History AZ-MO.csv"),
  read_csv("~/Downloads/2020 Pres County History/2020 Pres County History MT-WY.csv")
) |> 
  janitor::clean_names() |>
  mutate(timestamp = mdy_hm(timestamp, tz = "US/Eastern")) |>
  # remove newer timestamps since they're not useful to us and have some errors
  # see Maricopa for an example
  filter(month(timestamp) == 11, office == "President") |> 
...
write_csv(hist_20_merge, "~/Downloads/voteshift_2020_byCounty_raw.csv")
write_csv(hist_20_table, "~/Downloads/voteshift_2020_byCounty_rawPercent.csv")

hist_20_table |> 
  mutate(across(`0-10%`:`98-99.9%`, ~ `100%` - .x)) |>
  write_csv("~/Downloads/voteshift_2020_byCounty_relativePercent.csv")
...
<!-- selectInput("selected_state", "Select State:", choices = unique(hist_22_byCounty$state)) -->
<!-- uiOutput("county_ui") -->
<!-- checkboxInput("trend", "Trendline?") -->
...
<!-- # Update county based on the selected state -->
<!-- output$county_ui <- renderUI({ -->
<!--   req(input$selected_state) -->
<!--   counties <- hist_22_filter |>  -->
<!--     filter(state == input$selected_state) |>  -->
<!--     distinct(county) |> arrange(county) |> pull() -->

<!--   selectInput("selected_county", "Select County:", choices = counties) -->
<!-- }) -->

<!-- # Create the scatter plot -->
<!-- output$scatterPlot <- renderPlot({ -->
<!--   req(input$selected_state, input$selected_county) -->

<!--   filtered_data <- hist_22_filter |>  -->
<!--     filter(state == input$selected_state, county == input$selected_county) -->

<!--   p = ggplot(filtered_data, aes(x = time_since_close, y = vote_pct, color = candidate)) + -->
<!--     geom_point() + -->
<!--     labs( -->
<!--       title = paste("Votes Percentage Over Time in", input$selected_county, ",", input$selected_state), -->
<!--       x = "Time",  -->
<!--       y = "Percentage of Votes" -->
<!--     ) + -->
<!--     theme_bw() + -->
<!--     scale_x_continuous(labels = scales::label_timespan(unit = "hours")) -->
