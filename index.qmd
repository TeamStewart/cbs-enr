...
total = d1 |>
  ...
  scale_x_datetime(
    date_breaks = smart_datetime_axis(d1$timestamp),
...
mode = d2 |>
  ...
  theme_bw(base_size = 24) +
  scale_x_datetime(
    date_breaks = smart_datetime_axis(d2$timestamp),
    labels = minimal_datetime_labels
  ) +
  scale_y_continuous(
    labels = scales::label_comma(scale_cut = scales::cut_short_scale()),
...
data_hod_base = merge_data(
  ...
  add_obs("votes_governor_25_dem_precinct_total", "votes_potus_24_dem") |>
  select(-matches("votes_statehouse.*?_share$"), -starts_with("l_")) |>
  filter(
